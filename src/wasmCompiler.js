...
class API {
  constructor(options) {
    ...
    this.memfsFilename = options.memfs;
    ...
    this.memfs = new MemFS({
      ...
      hostWrite: (str) => { if (this.onOutput) this.onOutput(str); },
  ...
  async compileLinkRun(contents, options = {}) {
    const lang = options.language || "c";
    const input = lang === "c" ? "main.c" : "main.cc";
    const obj = "main.o";
    const wasm = "main.wasm";
    if (options.stdin !== undefined) this.memfs.setStdinStr(options.stdin);
    await this.compile({ input, contents, obj, language: lang });
...
export function createWasmCompiler(options) {
  const apiOptions = {
